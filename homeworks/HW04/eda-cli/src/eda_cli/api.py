...
@app.post(
    ...
    response_model=dict,
    tags=["quality"],
    summary="Оценка флагов качества по CSV-файлу с выводом флагов"
)
async def quality_flags_from_csv(file: UploadFile = File(...)):
    """
    Принимает CSV-файл и возвращает только словарь флагов качества данных
    """
    start = perf_counter()
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Ошибка при вычислении флагов: {str(e)}"
        )
    ...
    filtered_flags = {k: v for k, v in flags.items() if k in flag_keys}

    return {"flags": filtered_flags}
