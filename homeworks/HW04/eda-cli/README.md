...
---
### 5. `POST /quality-flags-from-csv` – только флаги качества по CSV-файлу

Эндпоинт предназначен для получения **исключительно набора булевых флагов качества**, включая кастомные эвристики, реализованные в HW03 (например, `has_constant_columns`, `has_high_cardinality_categoricals` и др.).

В отличие от `/quality-from-csv`, **не возвращает** `quality_score`, `message`, `latency_ms` или `dataset_shape` — только чистый словарь флагов.
